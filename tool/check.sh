...
echo "── 2/12 ignore 稽核(// ignore: 必須附 ' -- 原因')──"
# 生成產物豁免:/src/generated/ 與根 analysis_options.yaml 的
# analyzer.exclude 對齊;build/ 與 .dart_tool/ 為 build 產物,理由同 1/12(#40)
...
echo "── 3/12 護欄稽核(稽核稽核者;見 tool/guard.sh)──"
bash tool/guard.sh

echo "── 4/12 pubspec 依賴稽核(features 不得互依,packages 不得依賴 feature/app)──"
...
echo "── 5/12 bloc 純度稽核(bloc/event/state 不得 import Flutter)──"
...
echo "── 6/12 分層方向稽核(presentation 不得 import data 層)──"
...
echo "── 7/12 Firebase 隔離稽核(app 與 core 不得直接依賴 firebase_*)──"
...
echo "── 8/12 GetIt.instance 稽核(page 不得直接取全域容器)──"
...
echo "── 9/12 l10n 漂移檢查(ARB 需 regen 為 committed 產物)──"
...
echo "── 10/12 架構文件漂移檢查(拓撲/依賴表由 pubspec 產生)──"
